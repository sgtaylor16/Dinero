...
class InvestmentDB:
    ...
    def calcPortfolioValue(self,datestr:str) -> pd.DataFrame:
        """
    ...
    def readStatement(self,df:pd.DataFrame):
    ...
    def plotPortfolioValue(self, datestr:str) -> pd.DataFrame:

        df1 = self.calcPortfolioValue(datestr)
    ...
    def getPortfolioDates(self) -> list[str]:
    ...
    def invHistory(self) -> pd.DataFrame:

        #Get all dates in the assets table
        dates = self.getPortfolioDates()

        #For each date, calculate portfolio value
        outlist = []
        for date in dates:
            value = self.calcPortfolioValue(date)['value'].sum()
